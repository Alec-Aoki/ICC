...
void Abrir_Arquivo(FILE *pont_arq, aviao *voo, passageiro **pont_vet_passageiros, int *cont_passageiros, int *conot_comandos);
...
int main(void){
    //verifica se o voo nao esta fechado
    ...
    do {
        ...
        if(cont_comandos == 0){
           cont_comandos = 1;
           Abrir_Arquivo(pont_arq, &voo, &pont_vet_passageiros, &cont_passageiros, &cont_comandos);
        }

        /*ABERTURA DO VOO - AV*/
        if(strcmp(comando, "AV") == 0){
            if(fopen("voofechado.txt", "r")){
                Fim_Voo(&voo, cont_passageiros, pont_vet_passageiros, pont_arq);
            }else{
            Abrir_Voo(&voo);
            cont_comandos = 2;//Representa a abertura do voo e o funcionamento das funções
        ...
        /*REALIZAR RESERVA - RR*/
        else if(strcmp(comando, "RR") == 0 && cont_comandos == 2){
            if(fopen("voofechado.txt", "r")){
                Fim_Voo(&voo, cont_passageiros, pont_vet_passageiros, pont_arq);
            }else{
            cont_passageiros++;
        ...
        else if(strcmp(comando, "CR") == 0 && cont_comandos == 2){
            Consultar_Reserva(&voo, pont_vet_passageiros, cont_passageiros);
        }
          
        /*CANCELAR RESERVA - CA*/
        else if(strcmp(comando, "CA") == 0 && cont_comandos == 2){
            if(fopen("voofechado.txt", "r")){
                Fim_Voo(&voo, cont_passageiros, pont_vet_passageiros, pont_arq);
            }else{
        ...
        else if(strcmp(comando, "MR") == 0 && cont_comandos == 2){
            if(fopen("voofechado.txt", "r")){
                Fim_Voo(&voo, cont_passageiros, pont_vet_passageiros, pont_arq);
            }else{
    ...
    /*FIM DO DIA - FD*/
    if(strcmp(comando, "FD") == 0 && cont_comandos == 2){
    ...
    /*FIM DO VOO - FV*/
    else if(strcmp(comando, "FV") == 0 && cont_comandos == 2){
...
/*Função de cancelar reserva - CA*/
void Cancelar_Reserva(passageiro **pont_vet_passageiros, int *cont_passageiros){
    int posicao = -1; //Variável em que será guardada a posição do passageiro no vetor
    ...
    if(posicao == -1) return;//Caso nao haja reserva, nao ha cancelamento.
...
void Modificar_Reserva(aviao *voo, passageiro **pont_vet_passageiros,int cont_passageiros){
    ...
    if(posicao == -1) return;//Caso nao  haja reserva, nao ha modificação;
...
void Fim_Dia(FILE *pont_arq, aviao *voo, int cont_passageiros, passageiro *pont_vet_passageiros){
    ...
    int cont_comandos = 2;//se o codigo chegou até aqui, significa que o voo foi aberto, logo cont_comandos vale 2;
...
void Fim_Voo(aviao *voo, int cont_passageiros, passageiro *pont_vet_passageiros, FILE *pont_arq){
    int cont_comandos =2;
    //fopen retorna diferente de nulo se o arquivo existir
    if(fopen("voofechado.txt", "r")){
        //se o voo estiver fechado ele nao guarda nada (nao podemos editar um voo fechado)
    }
    else{
        //se o voo estiver aberto ele guarda tudo no arquivo "voo"
        pont_arq = fopen("voo.txt", "w");
        fprintf(pont_arq, "%d\n", cont_comandos);
        fprintf(pont_arq, "%d %d %f %f\n", voo->assentos_total, voo->assentos_disp, voo->preco_economica, voo->preco_executiva);
        fprintf(pont_arq, "%s %s %s %s\n", voo->origem, voo->destino, voo->data, voo->numero);
    ...
    //Aqui a gente so printa na tela
    ...
    //renomear o nome do voo para saber que ele vai estar fechado
    if(fopen("voofechado", "r")){
        //se o voo estiver fechado ele nao renomeia o arquivo (nao vai existir arquivo "voo.txt")
    }else{
      //caso o voo estiver aberto ele renomeia para sabermos que esta fechado  
    rename("voo.txt", "voofechado.txt");
...
void Abrir_Arquivo(FILE *pont_arq, aviao *voo, passageiro **pont_vet_passageiros, int *cont_passageiros, int *cont_comandos){
    //Lendo as informações do arquivo e jogando elas nos vetores

    //fopen retorna diferente de nulo se o arquivo existir
    if(fopen("voofechado.txt", "r")){
        //se o voo estiver fechado ele lee as informacoes do voo fechado
        pont_arq = fopen("voofechado.txt", "a+");}
    else{
        //se nao lee as informacoes do voo aberto
    ...
    */
    //Informações do voo:
    fscanf(pont_arq, "%d", cont_comandos);//serve para saber se o voo ja foi aberto, permitindo o funcionamento da funções.
